...
<<<<<<< HEAD
        ...
        if self.tag=='train':
            ...
            for i in grasp_pc:
                ...
                self.plys.append(i)
                self.label_list[i] = label[train_num][k]
        else:
...
>>>>>>> a9fc64760b36adc06d7d772e52c04514aa96e708
        ...
        if not self.projection:
            if len(obj) > self.grasp_points_num:
                ...
                obj = obj[np.random.choice(len(obj), size = self.grasp_points_num, replace=True)].T
        else:
            obj = obj.transpose((2,1,0))   

        #resized_obj = transforms.functional.resize(obj, [500, 3], interpolation=2)
        #resized_obj = obj.resize([500,3])
        #print( obj.shape)

        score = self.label_list[path]
        if score >= self.thresh_bad:
            label = 0
        elif score <= self.thresh_good:
            label = 2
